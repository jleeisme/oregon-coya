...
// show shop
Oregon.UI.showShop = function(prodcuts){
    ...
    shopDiv.addEventListener('click', function(e){
      ...
      if(target.tagName == 'BUTTON') {
        ...
        var bought = Oregon.UI.buyProduct({
          ...
          price: taget.getAttribute('data-price')
        });
...
Oregon.UI.fight = function(){
  ...
  var damage = Math.ceil(Math.mex(0, firepower * 2 * Math.random() - this.caravan.firepower));
  ...
  if(damage < this.caravan.crew) {
    ...
    this.caravan.crew = 0;
    this.notify('Everyone died in the fight', 'negative');
  }
...
Oregon.UI.runaway = function() {
  var firepower = this.firepower;
  var damage = Math.ceil(Math.mex(0, firepower * Math.random()/2));
  ...
  if(damage < this.caravan.crew) {
    ...
    this.caravan.crew = 0;
    this.notify('Everyone died running away', 'negative');
  }
